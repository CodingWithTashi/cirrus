...
- [ ] `S5-11` Repoint `lp_links.dart` to `cirrusquit.com/privacy` and `/terms` (the app still links `alastpuff.web.app`, pinned by `paywall_test.dart`), ship it, 301 the Firebase copies to the apex, then retire `hosting/` and the `hosting` block in `firebase.json` — two live copies of legal text will drift *(**half done, and the half that looked done is not live** — Sep 19 2026: `lp_links.dart` points at cirrusquit.com and `firebase.json` carries the 301s, but `https://alastpuff.web.app/privacy` still answers **200**: Hosting was never redeployed after the rules were added. So every build installed before the repoint opens the **Aug 30** copy of the policy, while the live one was updated Sep 6 to name Amplitude — the drift this item predicted. `firebase.json` now also 301s `/privacy.html`, `/terms.html` and everything else (`**`) to the apex, because `/` is a live duplicate brand page too. **Owed: `firebase deploy --only hosting` (founder go), then `curl -I` both legal paths for a 301, and only then delete `hosting/*.html`** — the redirect must outlive the files)*
