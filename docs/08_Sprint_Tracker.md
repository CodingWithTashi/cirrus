...
| **B4** | **RESOLVED Sep 2 (code).** RevenueCat is wired end to end: `purchases_flutter` behind `BillingRepository`, the Cirrus paywall opens the store sheet, Restore is real on the paywall and in Settings, `profile.tier` is gone from the journey and the client tier is `entitlementProvider`. Still open: the Play public key in `billing_options.dart`, the webhook secret + secret key in the RevenueCat dashboard, the `default` offering's `$rc_weekly` package, and the on-device sandbox run (S1-11). See §22. | `lib/data/repositories/revenuecat_billing_repository.dart`, `lib/data/stores/entitlement_store.dart`, `functions/src/handlers/rcWebhook.ts` | S1 |
...
- [ ] `S1-11` Sandbox purchase + restore verified **on both platforms** — blocked on §22's dashboard steps (public key, webhook secret, secret key, Play license tester)
...
| 18 | `onTrialWillEnd` (Doc 5 §7) | **Descoped.** RevenueCat sends no such event and neither store does; the reminder is deterministic from the entitlement's expiry, so it is scheduled on-device like the danger hours (`TrialReminderPlanner`) |
...
## 22. BILLING LANDS (Sep 2) — RevenueCat wired end to end
...
Docs/05's `onTrialWillEnd` is descoped (§7 #18); RevenueCat Paywalls and Superwall are not used (§7, S1-10).
...
## 23. THE PANIC ARCADE (Sep 2)

**Why.** docs/09 §8's tile game was one 60-second game. The evidence it leans on (Skorka-Brown, Andrade, Whalley & May 2015: three minutes of a visual game, cravings down about a fifth, consistent over a week; May, Andrade, Panabokke & Kavanagh 2010: any visual working-memory task suppresses cigarette craving, verbal ones do not) says the dose is three minutes and the mechanism is the load, not the game. The founder's brief: more than one game, each grounded in that research, professional game feel, the hook loop, and the craving drop as the reward — in the person's own numbers, never invented. Four decisions and four UX picks were put to the founder before a line was written; §7 rows 18–20 record the ones that bind.
