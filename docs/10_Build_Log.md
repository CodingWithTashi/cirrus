...
## 43. THE SITE NOBODY FINDS (Sep 19) — an audit of cirrusquit.com, and the start of the worldwide plan

Founder ask: how is the site doing, where is it failing, and then a plan to grow traffic around the world. Everything in 43.1 was read and nothing was changed: the Cloudflare dashboard, the live site over `curl`, and Play Console.

### 43.1 What the numbers say

| | Value |
|---|---|
| Real visits, 21 days (Cloudflare Web Analytics, bots excluded, **sampled to tens**) | **270** visits / 280 page views, ~13 a day |
...
The app ships five languages and is available everywhere. Nothing outside English can find it, and barely anything inside it. **The Overview tab is not the traffic number and must never be quoted as one.**

### 43.2 What was already right

`http` → `https` and `www` → apex both 301; HSTS and the other security headers come from `public/_headers`, so the dashboard's "Always Use HTTPS: off" is cosmetic; the live `robots.txt` is byte-identical to the generated one, so Cloudflare's managed prepend is off and the README saying otherwise was stale; AI *search* crawlers are deliberately allowed; "22% cached" is what Pages looks like (HTML is always `DYNAMIC`, hashed assets are `immutable`). Two things the first report got wrong and the repo corrected: the untagged App Store link is **deliberate** (the privacy policy says only Play links are tagged), and iOS attribution needs no token anyway — App Store Connect → Sources → Web Referrer names the domain.

### 43.3 Founder decisions (Sep 19 2026)

- **es, fr, de, pt — all four, phased.** Homepage, `/download` and both store listings first; blog after, top three posts per language. This pulls S10's listing work forward and adds Spanish, which no doc had planned.
- **es and pt are written for the wider audience** — neutral international Spanish and Brazilian Portuguese, keywords researched for those markets, calculator amounts as bare numbers rather than a guessed currency. That is *not* the app's dialect (Peninsular / European); the mismatch is logged in docs/08 §6 rather than hidden. fr and de reuse the ARB wording verbatim.
- **Claude drafts, the founder spot-checks.** No translator or reviewer is ever named on a page.
- **B20 is closed** — see the row. The apps moved accounts; the India payments constraint did not follow them.

### 43.4 The plan, one line per phase

- **0 — hygiene.** This section.
- **1 — the site in four languages.** Subdirectories with English unprefixed, and because of `build.format: 'file'` the locale homes are `/es`, never `/es/`. Astro's own `i18n` config stays off: its URL helpers do not check that a page exists, and this site has English-only legal pages. One typed dictionary per locale, so a missing key fails `astro check` — the site's `l10n_parity_test`. A `check-i18n` script also compares the **digit-runs** of every string against English, which is the honest-numbers rule applied to a translation. hreflang only where the page really exists; `LIVE_LOCALES` gates each language, so they can go live one at a time. Play campaigns become `es-hero`, `fr-blog-<slug>` — a prefix, frozen at the first tagged install.
- **2 — store listings**, versioned for the first time in `store_listing/` (starts closing B16). Submit is always the founder's click.
- **3 — blog.** Plumbing before content, because today a file dropped at `src/content/blog/es/x.md` would publish at `/blog/es/x` in English chrome. English slugs in every locale: a slug is the one part of a translation a spot-check cannot validate and cannot be fixed after publishing.
- **4 — English content.** `/vaping-cost-calculator` (the one docs/06 §5 item still missing) and sourced posts for the UK, Australia and Canada.
- **5 — off-site**, founder-side. No phase adds a tracker or touches the privacy policy.

### 43.5 Phase 0, landed on `site/worldwide-growth`

- **`og:title` / `twitter:title` fell back to the bare brand** (`BaseLayout.astro`), so every shared link to the home page read "Cirrus" and nothing else. They fall back to `pageTitle` now; post titles are unchanged.
- **The `#get` layout shift was `ch`.** `ch` is the advance of "0" in the font *currently rendering*, and the Arial fallback is narrower than the web faces even with `size-adjust`. Measured on production at 1920 px: `.cta-band h2` is 436 px wide under the fallback and 498 px under Space Grotesk, and its height goes **155 px → 104 px** on swap — the 51 px jump Cloudflare reported as CLS on `#get`. All fourteen `ch` widths are `em`/`rem` now (Inter 1ch = 0.631em, Space Grotesk Bold 1ch = 0.648em). The same measurement on the new build: **every delta is 0.0 px**, and the finished page is the same 7470 px tall as production. Text blocks also get `overflow-wrap: break-word`, because `body` is `overflow-x: hidden` and an overflow there is clipped, not scrolled.
  **The first version of this fix was wrong twice, and a desktop check could not have seen either.** The Pixel 8 stayed locked, so before production the old and new builds were served from one origin and compared in an iframe: 7 pages × 11 widths (320–1440 px), the height of every heading, paragraph, list item and table cell. (1) The tidy values — `34em`, `41.5rem` — were 1–2 px narrower than the `ch` they replaced, enough to push one FAQ answer onto an extra line at three widths; they are now the measured ratios to four decimals (Inter 1ch = 0.63086em). (2) `overflow-wrap` had been put on `th, td` as well. The compare table is `table-layout: fixed`, where a header word wider than its column overhangs harmlessly — and break-word snapped it instead: **"PREMIU / M"** at 360 px, every row taller. Table cells are out of the rule; tables scroll sideways already. After both: **77 of 77 identical.** A width change is verified across widths, or it is not verified.
- **`/get` answered every method with a 302 and a log line**, so scanners filled the redirect log with taps nobody made. GET and HEAD only now; anything else is a 405 carrying the same security headers, which had to be shared out of `redirectTo` because `_headers` cannot reach a Function's response. Run against `wrangler pages dev`: Android → Play with `utm_campaign=get`, iPhone HEAD → App Store, desktop → `/download`, POST and PUT → 405.
- **`npm run verify` (`scripts/check-dist.mjs`)** checks the *built* site, between Build and Deploy: one canonical per page equal to its URL, internal links and `#anchors` resolve, the sitemap agrees with what is indexable, and the ONE QUERY ONE PAGE check the README described and nobody had written. **It failed on its first run, on a real collision:** "Do puff counter apps actually work?" is `FAQPage` schema on both `best-quit-vaping-apps` and `how-to-choose-a-puff-counter-app`, with near-identical answers. Founder call: the puff-counter post owns the query (it is that post's whole subject, and the home FAQ already hands off to it), so the entry came out of `best-quit-vaping-apps` — accordion item and schema together, since both render from the one frontmatter entry — and its `updatedAt` moved to Sep 19 so the sitemap asks for a recrawl. The JAMA source stays: the post's closing paragraph still makes the claim it backs.
- **The Firebase 301s were never live.** `https://alastpuff.web.app/privacy` answers 200: Hosting was not redeployed after the rules were written, so every build installed before the repoint still opens the Aug 30 policy while the live one was updated Sep 6. `firebase.json` now also sends `/privacy.html`, `/terms.html` and everything else to the apex. **Deployed Sep 19 on the founder's go** (`firebase deploy --only hosting`, nothing else) and read back on both `alastpuff.web.app` and `alastpuff.firebaseapp.com`: `/privacy` and `/terms` 301 to their exact pages in one hop, the rest to the apex.
  **The catch-all is `!/__/**`, never `**`, and the first deploy got that wrong.** Hosting's reserved namespace is supposed to outrank redirects, and `/__/auth/handler` did keep answering 200 — but `/__/firebase/init.json` started answering **301 to cirrusquit.com**, and `handler.js` reads that file. Nothing in the app uses the web handler today (Google is native on Android, Apple is native on iOS), so nothing broke; the first provider added through the generic OAuth flow would have failed for a reason nobody would have looked for in a redirect rule. Redeployed within minutes; `init.json`, `handler`, `iframe`, `action` and `handler.js` all read 200 again. `hosting/*.html` is now unreachable and can be deleted in a later change — the redirect outlives the files.
- Stale notes fixed: `wrangler.jsonc` ("no Pages Functions any more" — there is one), the README's managed-robots section, `_headers` (`/go/*` is reserved, not claimed), the README's `public/blog/` image paths (they live beside the posts).

### 43.6 Gates

`npm run check` 0 errors / 29 files · `npm run build` 17 pages · `npm run verify` red on the FAQ collision first, then **17 pages, 368 internal links, 51 FAQ questions — all consistent** · `/get` exercised on the local Pages runtime · CLS measured in Chrome, production against the new build. **Preview deploy** (`preview-site-worldwide-growt.cirrus-landing.pages.dev`, run 35456301792): Typecheck, Build and the new *Verify built site* step all green on CI, and the four changes read back off the real edge — full `og:title`, no `ch` in the shipped CSS, `/get` 302 on GET and 405 on POST, the duplicate FAQ gone from one post and present on the other. **Not done:** the phone-width pass — the Pixel 8 was locked the whole session, so nothing was seen on the device.

### 43.7 The Cloudflare zone — two changes made, five handed over

- **Done:** the stray `txt.cirrusquit.com` TXT record is deleted (a typo'd copy of the apex SPF; nothing sends mail from that name). Read back from `1.1.1.1`: NXDOMAIN, with the apex SPF, the Google verification and all three MX records intact. To restore: name `txt`, TXT, `v=spf1 include:_spf.mx.cloudflare.net ~all`, TTL Auto, DNS only.
- **Founder-side, and the plan was wrong to say otherwise:** the POST-block WAF rule, Bot Fight Mode, Minimum TLS 1.2, Always Use HTTPS and DMARC are *security settings*, and Claude's browser automation may not change those even with a yes. The plan promised "through Chrome with a yes per action"; that holds for nothing in the Security or SSL/TLS sections.
- **Crawler Hints was stopped at the toggle the first time.** Enabling it is an agreement, not a switch: the card says doing so accepts Cloudflare's Supplemental Terms and shares site information with crawlers. The founder had approved a toggle, not terms nobody had read — so it went back to them with that stated, and came back as a yes. **On since Sep 19 2026**, read back after a reload; Always Online and Development Mode beside it untouched. It is what sends IndexNow pings to Bing, Yandex, Seznam and Naver when a page changes.

## 44. FIVE LANGUAGES' WORTH OF STRUCTURE, ONE LANGUAGE LIVE (Sep 19) — phase 1(a) of §43, a refactor proven invisible

The site can now be asked for a language; it is still only ever asked for English. This is the step §43.4 called the skeleton: everything es / fr / de / pt will drop into, landed with `LIVE_LOCALES = ['en']` so that not one URL, tag or sentence changes. Branch `site/i18n-skeleton`, stacked on §43's.

### 44.1 What moved where

- **`src/i18n/locales.mjs` is the one list.** Locales, hreflang / `og:locale` / `Intl` tags, the calculator's currency — and `LIVE_LOCALES`, which routes, hreflang, the sitemap and the switcher will all read and nothing else will. Plain `.mjs` because `astro.config.mjs`, `scripts/`, `functions/` and the TypeScript all need the same list and only that is importable from all four.
- **`src/i18n/en.ts` is English and the shape.** Other locales are declared `: Dictionary`, so a missing key is TS2741 and an extra one TS2353 — `npm run check` becomes the site's `l10n_parity_test`.
- **`src/lib/content.ts` holds what is not language** — ids, order, tiers, tones, 24-hour clock times — and the dictionary is `Record<StatId, …>`, `Record<FaqId, …>`. A locale cannot ship six timeline steps or a statistic without its `source`: the honest-numbers rule is a type now. It also retires a trap: the home page chose its three stats by matching the English figure (`'76%'`), which a French `76 %` would have silently dropped.
- **Page bodies live in `src/components/pages/`**; `pages/index.astro`, `download.astro` and `404.astro` are five-line wrappers that did not move, so App Links' claim on `/download` and the site root cannot regress.
- **`BaseLayout` takes `lang` and `alternates`.** `<html lang>`, `og:locale`, JSON-LD `inLanguage`, the chrome and the breadcrumb root follow the language; hreflang is emitted only for a page with two or more live versions, never on `noindex`, and **a self-reference that is not the page's own canonical path is a build error**. The header's "Get the app" tested `cleanPath === '/'`, which would have sent the Spanish home page's button to the English `/download`; it tests the locale home.
- **The calculator's runtime strings ride in a JSON data island** (`#demo-i18n`). Astro bundles a component's `<script>` once for every page, so it has no `lang` and cannot import a dictionary; `define:vars` would have forced it inline on every page. Its currency is config (`DEMO_CURRENCY`): USD for English, EUR for fr/de, **a bare number for es and pt**, which serve two continents each.
- **`campaignFor(lang, base)`** — English campaign names untouched (installs cannot be re-attributed), everything else prefixed: `es-hero`. `link()` in `i18n/paths.ts` answers with the localized URL when that page exists and the English one flagged `hreflang="en"` when it does not; Astro's own `i18n` config stays off because its helpers never ask.

### 44.2 Proving a refactor changed nothing

Byte equality is the wrong test: moving a sentence from template text into an expression turns `'` into `&#39;` and drops the source's line wrapping, and neither is visible to anything. So each page of the new build was reduced to what a browser, a reader and a crawler actually get — the sequence of tags with their attributes, the text between them with entities decoded and whitespace collapsed, and the JSON-LD parsed and re-serialised, with hashed asset names normalised — and compared with §43's build (`npm run compare`, `scripts/compare-dist.mjs`, kept in the repo so the claim can be re-run and because the blog's language plumbing needs the same gate). **17 of 17 pages structurally identical; the one addition is the data island on the home page.** The comparison was then run against a copy with one word, one `aria-label` and one JSON-LD field altered, and named all three — a check that cannot fail proves nothing.

What that comparison cannot see is behaviour, so the calculator was driven in Chrome off the new build: all five dependence bands and the empty state, the keypad, the stepper, both directions of the puffs/disposables toggle, 35 × 52 = `$1,820`, and the Continue link still tagged `hero-demo`.

### 44.3 Gates

`npm run check` 0 errors / 36 files · `npm run build` 17 pages · `npm run verify` 17 pages, 368 links, 51 FAQ questions · `wrangler pages functions build` compiles (`store.ts` now imports the registry, and `/get` imports `store.ts`) · `/blog/` and `/blog.html` read 308 → `/blog` on production, which is the behaviour `/es` will inherit. **Not done:** no dictionary but English exists, no `[lang]` route, no switcher — and the switcher is new design, so it gets a preview and a go before it is built. **PR #11 (§43) is not merged**: the merge was refused to Claude as a merge without review, so production still serves the pre-§43 site.

## 45. FOUR LANGUAGES WRITTEN, NONE PUBLISHED (Sep 19) — §43 goes to production, the zone is hardened, and es / fr / de / pt wait on a read

### 45.1 What went live

- **§43 is on cirrusquit.com.** The founder merged #11; the production deploy ran from `main` and was read back off the live site: the full `og:title`, no `ch` in the shipped CSS, `/get` 302 on GET and 405 on POST, the duplicate FAQ gone from one post and kept on the other, sitemap and `assetlinks.json` intact.
- **#12 merged into its base branch, not into `main`** — eleven seconds after #11, never retargeted. The stacked-PR trap. No harm: `main` held exactly §43, which is what production was meant to get, and §44 is output-identical anyway. #13 carries it across.
- **The five zone settings are the founder's clicks, and all five are in.** Claude opened each page and left the clicking; then checked from outside and in the dashboard: `POST /` → **403** with `GET /` still 200; Bot Fight Mode on; Always Use HTTPS on; Minimum TLS **1.2**; `_dmarc` → `v=DMARC1; p=none; rua=…@dmarc-reports.cloudflare.net`. After Bot Fight Mode: Google's Digital Asset Links API still returns both statements for `com.quitvape.last_puff`, and eight link-preview user agents (Facebook, X, Slack, WhatsApp, Discord, LinkedIn, Telegram, Reddit) get 200 for the page and the card. Those were sent from one IP with borrowed strings — the security events over the next two days are the real test.

### 45.2 The translations

Four dictionaries, 210 strings each, **drafted by Claude and not yet read by anyone who speaks them** — which is why none is published. fr and de copy the app's vocabulary verbatim from the ARB files, and reading those corrected the plan: the app says **taffes**, not *bouffées*; *cravings* stays English in fr, de and pt; the button is `TAFFE +1` / `ZUG LOGGEN`. es and pt are written for the wider audience (§43.3): neutral Spanish with `tú`, "teléfono", "computadora"; **Brazilian** Portuguese with `você`, "celular", "tragadas", and **"fissura"** — the word Brazilian cessation material uses for a craving. The pt page never quotes the app's European button label at a Brazilian reader; it describes the button. Address is kept genderless where the language allows it, as the app's is.

- **`scripts/check-i18n.mjs` found four mistakes in its author's own drafts on its first run**: three meta descriptions over 160 characters (German at 180) and a digit with no English counterpart. **Digit parity is the honest-numbers rule applied to a translator** — every run of digits must match English after separators are stripped, so `2,588` = `2.588` = `2 588` and `$2.99` = `2,99 $`, and a figure English spells out cannot become a numeral. Two exemptions, each written down with its reason: fr/de say `15 h` / `15 Uhr` where English says `3pm`, and the French button label really is `TAFFE +1`.
- **An invisible-character trap.** A typed `\u00a0` does not survive the Write tool: it lands as the raw character. `fr.ts` had 60 no-break spaces nobody could see under a comment promising the opposite, the check script had five, and the two earlier scripts one each. All are visible escapes now, and the script refuses U+202F outright — Space Grotesk has no glyph for it, so it is a box on a social card.

### 45.3 Building a language without publishing it

`LIVE_LOCALES` is two lists: **`PUBLISHED`** (what cirrusquit.com serves — still `['en']`) plus **`CIRRUS_PREVIEW_LOCALES`**, an environment variable the workflow sets to `es,fr,de,pt` for the `preview` target only. Cloudflare sends `X-Robots-Tag: noindex` on preview deploys — checked, and absent on production — so a translation can be read on a real URL and a real phone with no way into an index. `process` is guarded in the registry because the same file is bundled into the `/get` Worker, which has none.

- **The switcher is option A** (the founder's pick from the preview): the language **code** in a native `<details>` from 30rem, and a row of autonyms in the footer at every width. No script, no cookie. The footer's grid rule was `:last-child { justify-self: end }`, which the new row would have stolen; it names `.footer-links` now.
- **`check-dist` learned hreflang**: `<html lang>` matches the URL; a page with alternates lists itself at its own canonical; x-default is English; every target exists, is in that language and lists the page back; none on `noindex`; the sitemap's alternates equal the `<head>`'s; and a localized page may link an English URL that has a translation only if the link carries `hreflang`. Tampered copies — a wrong `lang`, a dropped reciprocal link, an unmarked English link — were each named.
- **The sitemap's lastmod lookup would have handed `/es/blog/<slug>` the English post's date**, and its date scrape read the whole file, where one post shows a second `publishedAt:` inside a code fence. Anchored to the path and scoped to the frontmatter block, before the first translated post exists.

### 45.4 Gates

`npm run check` 0 errors / 45 files, `check-i18n` 5 locales × 210 strings · **English-only build: 17 pages, 17 of 17 structurally identical to §44's** (`npm run compare`) — production cannot change until `PUBLISHED` does · **preview build: 29 pages**, 748 internal links, 95 FAQ questions, 10 pages with hreflang, 23 sitemap URLs with alternates on the 10 and none elsewhere · layout driven in Chrome across 5 languages × 2 pages × 6 widths (320–1280): **no sideways scroll and no overflowing text in any language**, German included; the only flag is the header button wrapping to two lines at 320 px in en / fr / pt, which English already does on production. The German page was read by eye: `1.040 €`, `20 € pro Woche`, 24-hour times, the switcher panel floating with the page height unchanged. **Preview deploy** `preview-site-i18n-locales.cirrus-landing.pages.dev` (run 35461597582: Typecheck, Build 29 pages, Verify, Deploy all green on Linux / Node 22, so the type-stripped import of the dictionaries works in CI too), read back off Cloudflare's edge: `/es` 200, `/es/` and `/es.html` 308 → `/es`; a bad URL under `/es` answers **404 with the Spanish page**, and the English one elsewhere; `x-robots-tag: noindex` on every response; the French page tags Play `fr-hero`, `fr-closing`, `fr-hero-demo`; and production is untouched — `cirrusquit.com/es` is a 404 and the home page carries no hreflang.

**Pixel 8, the preview, by hand:** the German home and the Portuguese one. The header holds brand, Blog and "App holen" on one row with no switcher, as designed; `hyphens: auto` breaks "runter-bringt" at the syllable; Play's badge leads because the phone is Android; the footer's five language names fit one line with the current one lit, and the English-only links carry their orange `· EN`. Chrome offered "Translate page? German to English", which is the browser agreeing about `<html lang>`.

**Not done:** the screenshot rail and both store badges are still the English artwork (translated alt text and accessible names); per-locale social cards; **any read by someone who speaks these languages**, which is what `PUBLISHED` is waiting for.

## 46. FOUR LANGUAGES GO LIVE (Sep 19) — a gate that worked, and the founder's call to open it

The founder merged #13 and #14, deployed production, opened cirrusquit.com and asked where the language option was. It was not there **by design**: `PUBLISHED` was still `['en']`, so production built no switcher, no `/es`, no hreflang — the gate doing exactly its job, and looking, from outside, exactly like a feature that had not shipped. Asked which to publish, with the caveat stated (Claude's drafts, numbers machine-checked, idiom unread by a native speaker): **all four, now.** Reach first; phrasing gets fixed as it is spotted.

- The change is one line — `PUBLISHED = ['en', 'es', 'fr', 'de', 'pt']` — and everything else follows from it: 29 pages where there were 17, the switcher on every page, hreflang with x-default on the ten pages that have versions, 23 sitemap URLs with alternates on those ten. Built with **no** `CIRRUS_PREVIEW_LOCALES` set, so the gates ran on precisely what production builds: `npm run check` 0 errors and 5 locales × 210 strings, `npm run verify` 29 pages / 748 links / 95 FAQ questions / 10 with hreflang.
- **To take a language back down:** remove its code from `PUBLISHED` and deploy. Its routes, hreflang entries, sitemap alternates and switcher row go with it; the URLs then 404, so do it soon or not at all once Google has them.
- **A lesson for the next gate:** a switch that hides a finished feature needs saying out loud at hand-over, in the first line, not in the PR body. "Nothing is published" was written in three places and still read as "it is deployed".

## 47. A ROAST, CHECKED BEFORE IT WAS ANSWERED (Sep 19) — the home page names the problem before it sells the fix

The founder posted the pitch to a Reddit feedback thread and brought back five replies. Each was checked against a primary source or the live data before any copy moved (branch `site/landing-revamp`, preview first, production on the founder's go).

| Reply | Verdict | What changed |
|---|---|---|
| "Severe lack of emotional friction… hit their trembling pockets and scarred lungs" | **Half right.** The page led with a method and a clever line ("One bad Tuesday…", "the wrist it's attached to") and kept its three sourced figures near the bottom under a heading about *us* ("Every number here has a source"). But "scarred lungs" is not an established harm of nicotine vaping, and fear with no way out beside it pushes dependent readers away rather than toward the download (see below). | The sourced figures moved up to sit straight after the cost calculator, each under a line the reader recognises ("It's in your hand before you're out of bed.", "You've quit before. It didn't stick.", "The craving feels endless. It isn't."), under "It stopped being a choice a while ago." The last card is the way out. The honesty line became a footnote. Money: "it stings exactly as much as it should" above the calculator, "a year you keep once you stop" under the figure — the reader's own arithmetic, no new number. |
| "CDC's own estimate is 11–12M, not 17.8" | **The commenter is out of date; the post was loosely cited.** 11.1M is CDC's 2021 headcount (MMWR 72(18)). 17.79M (7.0%) is the 2024 NHIS, but the headcount is an independent analysis (Agaku, *NEJM Evidence*, Mar 2026), not a CDC publication. "Nearly double since 2020" is arithmetically true (9.1M → 17.8M) and cherry-picked: 2020 was a dip below 2019's 10.9M. | Nothing on the site: neither figure was ever on it, and a market size is a pitch number, not a reader's. |
| "Your app doesn't count puffs, you tap a button — that's a diary" | **Right about the words.** "Counts your real puffs" (meta description, /download) implied automatic counting. Some refillable devices do keep their own puff count; no phone app can see a vape. | Every "counts your real puffs" is now "one tap logs a puff". New home FAQ "Does Cirrus count vape puffs automatically?" says no, says what the count is measured against, and answers the "vape puff counter" / "vape hit counter" searches the site already appears for. Hero chip: "Works with any vape". |
| "Repair tokens are Duolingo freezes with extra steps" | **Fair, and useful**: the comparison explains the feature faster than the name does. | The example day now says "a streak freeze you had to earn: seven days under your limit bought a repair token". "Seven clean days" was also wrong — a token is earned by days *under the limit*, not days with zero puffs. |
| "Do the 2am searches convert, or do people bounce off the calculator?" | **Too early to know — search sends almost nobody yet.** Search Console, 28 days to Sep 16: 456 impressions, **2 clicks**, average position 38. Both clicks went to the most-dangerous-vapes post; the home page had 12 impressions. | Home `<title>` now leads with the query: "Quit vaping app without going cold turkey · Cirrus" (was "Cirrus · Quit vaping…" — no "app" anywhere in it). `/download` was "Get Cirrus · Cirrus". |

**The evidence behind the tone call** (a literature pass run for this change; abstracts or full text read, PMIDs in the session record):

- **Fear works, modestly, and only beside a way out.** Tannenbaum 2015 (*Psychol Bull*, 248 samples): d≈0.29 overall, about twice as large with an efficacy message as without, and no gain from moderate to high fear. Witte & Allen 2000: more fear, more defensive reaction, worse when efficacy is weak. Peters, Ruiter & Kok 2013: threat helped behaviour only when efficacy was high. So the cards are blunt, the last one is "it ends", and the example day follows straight after.
- **Shame costs the readers who most need the page.** Stigmatising ads lowered quit intent in smokers with low confidence, and shame carried the effect (Kim 2018). The failed-attempts card is aimed at exactly that reader, so its hook says "It didn't stick", not a joke at them. (A little survey work links felt stigma to *more* quit intent, so "no shame" is the safer rule, not a proven one.)
- **Framing is a small lever; gain for the money.** Gain frames edge loss frames for cessation (Gallagher & Updegraff 2012, r≈.08). Young adults who vape rated money the most motivating message (Romm 2024). The year figure now reads "a year you keep once you stop".
- **"Scarred lungs" is not established for nicotine vapes.** Established: dependence, cough and wheeze in cohort studies, raised heart rate, metals in the aerosol, unknown long-term effects (NASEM 2018, CDC). Not established: popcorn lung, EVALI from nicotine products (82% of EVALI patients reported THC), "causes lung disease". The page states none of the second list.
