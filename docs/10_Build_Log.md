...
`npm run check` 0 errors / 45 files, `check-i18n` 5 locales × 210 strings · **English-only build: 17 pages, 17 of 17 structurally identical to §44's** (`npm run compare`) — production cannot change until `PUBLISHED` does · **preview build: 29 pages**, 748 internal links, 95 FAQ questions, 10 pages with hreflang, 23 sitemap URLs with alternates on the 10 and none elsewhere · layout driven in Chrome across 5 languages × 2 pages × 6 widths (320–1280): **no sideways scroll and no overflowing text in any language**, German included; the only flag is the header button wrapping to two lines at 320 px in en / fr / pt, which English already does on production. The German page was read by eye: `1.040 €`, `20 € pro Woche`, 24-hour times, the switcher panel floating with the page height unchanged. **Not done:** the screenshot rail still shows the English store images under translated alt text; per-locale social cards; any read by a speaker; and nothing has been seen on a phone — the Pixel 8 has been locked all day.
