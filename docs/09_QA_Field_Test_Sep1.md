...
- **Follow-up (not this issue):** surface `bestGameScore` in the coach's week card — `functions/src/domain/journeyCodec.ts` would decode it and `functions/src/ai/memoryCard.ts` quote it. Server-side, separate change.
