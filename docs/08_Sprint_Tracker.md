...
| ~~**B14**~~ | [RESOLVED Sep 4 2026 — Android] **Home-screen widget shipped.** `CirrusWidgetProvider` reflows 2x2→4x2, shows the day number above the count, and logs a puff with one tap while the app is dead — the taps queue in a single-writer outbox and drain through the same `JourneyStore.logPuff(at:)` the in-app button uses, so there is no second copy of the counting maths. Verified on a Pixel 8: tap with the process cold, clamp at zero, exact drain, idempotent across three relaunches, queue pruned. **Android has no phone lock-screen widget** (Google removed them in 5.0; Android 14 brought them back tablet-only), so the design's two frames collapse into one resizable widget. **iOS landed Sep 5 2026** — see `B22`. | `test/android_widget_test.dart` (19), `test/data/widget_drain_test.dart` (14), `test/ios_widget_test.dart` (21), `test/ios_widget_contract_test.dart` (4, macOS) | S0 done · both platforms |
...
| **B16** | ~~No CI~~ **CI done** (S0-21 — `.github/workflows/ci.yml`: Flutter analyze + test + l10n-drift check, functions `verify`, an emulator job for the rules + integration suites). ~~Default launcher icons~~ **branded icons done** Aug 30 (`flutter_launcher_icons`, S0-22); **re-cut Sep 5** from the final art (`assets/images/icon-square.png` / `icon-rounded.png`, adaptive inset 19%, themed silhouette via `tool/monochrome_icon.py`). **Open: no fastlane, no store assets.** | `.github/workflows/ci.yml`; `pubspec.yaml` `flutter_launcher_icons:` | S0 · open: fastlane + store assets |
...
| **B23** | **Push and the in-app inbox were both dead in production, for different reasons (docs/10 §31, founder report Sep 7).** Neither was a coding gap — the send path, gate, collapse, channels, taps and copy were all written, tested and deployed. **(a) The inbox:** a `fieldOverride` in `firestore.indexes.json` REPLACES Firestore's automatic single-field indexes rather than adding to them, so the override added for `pruneOldNotifications`' collection-group sweep listed only ASCENDING and deleted the COLLECTION_DESC index the app reads through. Every inbox read answered `FAILED_PRECONDITION`, and `NotificationsStore` treats a stream error as an empty inbox by design — so the screen showed its empty state and the bell never badged, silently, with the emulator suites green throughout (it does not enforce single-field indexes). **(b) The reply push:** `notifThreads/{postId}` has two writers, and `syncUserContext`'s read-marks create it holding only `{seenAtMs}` — the ordinary shape once an author has opened their own post. `notifyAuthor` cast that row to `ThreadNotifState`, so every arithmetic field was `undefined`: it decided `send: false` and then threw on the write, which `notifyReply`'s catch swallowed, so no push, no `notifiedAt` and no inbox row either. Production held exactly one reply that proved it. **(c)** the inbox marks 50 rows read and the server capped the list at 20, so the badge came back on every launch. **(d)** a returning user on a NEW device is never asked for POST_NOTIFICATIONS - onboarding is skipped for a restored journey and the Settings sheet had no `notAsked` branch - so no FCM token could ever exist; **(e)** the unread badge set its digit in `emberText`, which is byte-identical to `ember` in Midnight, so the count was orange on orange and invisible (`onEmber` added to all six palettes). **RESOLVED Sep 7: deployed (indexes + all 24 functions) and verified end to end on a Pixel 8 against real Firebase** - push delivered on channel `community_replies`, inbox row rendered, badge showed 1, tap opened the thread, read-mark survived a restart, second reply re-badged live. | `test/firestore_indexes_test.dart`, `functions/test/notifyCollapse.test.ts`, `functions/test/integration/triggers.test.ts` (2 new, red before the fix) | S0 — resolved |
| ~~**B24**~~ | [RESOLVED Sep 13 2026 — working tree, verified on a Pixel 8] **Five founder reports from day two of a test journey (docs/10 §40), each reproduced with a throwaway probe before anything changed.** (1) "$0 saved" until the first puff: the confirmed-day rule is kept and now explained by one savings (i) sheet on Home and Money. (2) Stats waited for two days on the books: every card renders from day one, Week/Month are 7/30 slots with bars capped at 26/9dp and counts on them, the Day view starts at midnight, and the caption no longer claims a recovery nobody had. (3) Authors could react to their own posts: no row until someone else reacts, then read-only counts, and since docs/10 §41 the rules refuse it too (`!isAuthorOf(postId)` on `postAuthors`; 0 self-reactions found in production) — **rules deploy pending**. (4) A slow launch read sent signed-in users to sign-in: the cached copy restores, then a splash retry — never sign-in. (5) A notification opened its thread without the reply, from two causes: a feed load overwriting a later thread read, and a feed landing mid-read leaving a reply-less duplicate the screen found first (the second caught only on device). Follow-ups in docs/10 §41: an honest nicotine card, Undo and past-day edits moving "last puff", "vs day 1" null instead of 0, no "I got you" on your own SOS. App and rules; **not committed**, rules **not deployed**. | `test/widgets/savings_info_test.dart`, `stats_day_one_test.dart`, `own_post_reactions_test.dart`, `session_restore_test.dart`, `test/data/community_thread_race_test.dart`; full suite 1881 green; device table docs/10 §40.6 | S2 |
...
- [~] `S0-25` *(iOS submission)* **App Store rejected the Sep 7 2026 submission for a missing Terms of Use (EULA) link** — guideline 3.1.2, an automated check on the product page: an app selling auto-renewables must link the EULA in its metadata and in the binary, and neither the privacy-policy URL nor `cirrusquit.com/terms` counts (Apple means its standard agreement unless a custom one is filed). Fixed on all three surfaces: the App Store description ends with Privacy Policy, Terms of Use and Apple's standard EULA URLs; the app links Apple's EULA beside Terms on the sign-in footer, the paywall and a Settings row, on Apple platforms only (`LpLinks.appleEula`, `appleEulaApplies`; pinned by `test/widgets/paywall_test.dart` and `test/core/lp_links_test.dart`); `/terms` names Apple's EULA for App Store installs. Same pass, the rest of the common-rejection list checked against the repo (docs/10 §34) — one gap closed: 5.1.2(i) third-party-AI disclosure, now on the coach-name step (`obCoachNameAiNote`) and under the coach composer (`coachSafetyNote`). **Open (founder):** the description edit alone clears the rejection; upload a build carrying the in-app link (`flutter build ipa`) and resubmit, and push `cirrus-landing` so the terms change deploys. **Also open (founder, App Store Connect → App Privacy):** the label declared only Name and Email, with Email marked *used for tracking* — a tracking claim with no App Tracking Transparency prompt is its own 5.1.2 rejection. The privacy-policy URL is repointed to `cirrusquit.com/privacy` and seven missing data types are selected (Health, Other User Content, User ID, Device ID, Purchases, Product Interaction, Crash Data) but each still needs its usage answers and the label a **Publish** — the answers to give are in docs/10 §34
- [~] `S0-26` *(iOS submission)* **App Store rejected 1.0.16 (17) on Sep 11 2026 — three issues** (docs/10 §38). **5.6.3, fixed in code:** the rating ask was onboarding step D3, which the guideline forbids outright (no rating request on first launch or during onboarding, whatever the copy). `ObStep.rating`/`RatingStep` deleted; the ask lives on the Survived screen behind `ReviewAskPolicy` (plan day ≥ 3, ≥ 3 cravings survived, twice ever, 14 days apart) plus a user-initiated **Rate Cirrus** row in Settings that opens the store's review page; `test/review_ask_placement_test.dart` reads the sources so it cannot come back. **2.3.2, App Store Connect:** the same 1024² shield image was the *Image* on all three subscriptions; promotion cannot even be enabled — the binary lacks the StoreKit `PurchaseIntent` API — so the images serve nothing today. Original backed up to `~/Downloads/cirrus_promo_original_1024.png`. **2.1, a question:** yes, Ember runs on Google Gemini server-side — **the reply was sent** Sep 11 (docs/10 §38 has the wording), answering it in full. **The duplicate promo image is fixed** (Sep 11): removing the subscriptions from the draft flipped the images from `WAITING_FOR_REVIEW` to editable, and they were deleted from weekly + monthly — only yearly keeps one. **Open (founder):** `flutter build ipa` + upload the build with the 5.6.3 fix, re-add the subscriptions to the draft, then **Resubmit to App Review**.
...
- [x] `S0-22` Branded app icons both platforms; replace default Flutter icons (B16) *(landed Aug 30 — `flutter_launcher_icons`, both platforms; re-cut Sep 5 from the final art in `assets/images/icon-square.png` + `icon-rounded.png` — docs/10 §26)*
...
- [~] `S1-2` Register App Check (Play Integrity + App Attest) — every callable enforces App Check (one flag since Sep 7: `ENFORCE_APP_CHECK` in `functions/.env.alastpuff`, docs/10 §33), so without this **every call is rejected**; without it enforced, `aiCoachChat` is a public Gemini proxy *(release providers wired in `app_check_setup.dart` and debug tokens registered for both apps (B3, B6); the release fingerprints / App Attest registration in the console is unverified (S0-24))*
...
- [~] `S3-11` SOS: the 60-min pin is **done**, the panic flow routes into it (composer pre-tagged `sos`), and since Sep 6 2026 a reply notifies the post's author on EVERY tag, not just SOS — collapsed per thread, with `@alias` mentions and a deep link to the thread (docs/10 §30). Since Sep 7 2026 there is a **picker** for those mentions (docs/10 §32) — the pipeline had shipped, been tested and been deployed while still requiring the user to type `@quietfox42` from memory, so nobody ever used it — and a named reply is now addressed to the people it names, the author hearing only when named or when nobody is. **An SOS is exempt from that narrowing**, because the content of `sosReply` is how many people came. Still open: notifying the last-5 responders (a fan-out that wants its own decision now the pipeline exists) and the "23 people had your back" count (Doc 3 §9). The buddy half of this line is dropped with the buddy system
...
- [x] `S4-3b` **The 3/day cap is scoped to nudges we invent, deliberately.** It is enforced on the device by `ReminderPlanner.maxPerDay`, which the server cannot see. Interpersonal push (a reply, a mention) has its own budget, `DAILY_PUSHES` = 10, on `users/{uid}.pushUsage` — a reply is an answer, not a nudge, and an app that rations answers at three a day is not a community. Collapse already keeps one busy thread to two-to-four sends. **This diverges from docs/03 §8's words "max 3 pushes/day total" and is recorded here as the resolution** (Sep 6 2026)
...
- [ ] `S5-11` Repoint `lp_links.dart` to `cirrusquit.com/privacy` and `/terms` (the app still links `alastpuff.web.app`, pinned by `paywall_test.dart`), ship it, 301 the Firebase copies to the apex, then retire `hosting/` and the `hosting` block in `firebase.json` — two live copies of legal text will drift *(**half done, and the half that looked done is not live** — Sep 19 2026: `lp_links.dart` points at cirrusquit.com and `firebase.json` carries the 301s, but `https://alastpuff.web.app/privacy` still answers **200**: Hosting was never redeployed after the rules were added. So every build installed before the repoint opens the **Aug 30** copy of the policy, while the live one was updated Sep 6 to name Amplitude — the drift this item predicted. `firebase.json` now also 301s `/privacy.html`, `/terms.html` and everything else to the apex, because `/` was a live duplicate brand page too — through `!/__/**`, never `**`, which also swallowed the `/__/firebase/init.json` that Auth's web handler reads (docs/10 §43.5). **Deployed and read back Sep 19 2026** on both Firebase domains: legal paths 301 to their exact pages, the rest to the apex, the `/__/` namespace untouched. **Still owed: delete `hosting/*.html`** (unreachable now) — the `hosting` block itself stays, because the redirects live in it and must outlive the files)*
...
- [x] `S5-37` **Cirrus on the wrist** (`docs/10 §29`): a watchOS app and a watch-face
  complication, on a founder decision that supersedes §7 row 9's "V2". A third renderer of the
  same mirror and a third producer into the same outbox — `CirrusShared.swift` and
  `CirrusOutbox.swift` are members of four targets now, so no number is computed twice. The one
  new problem is that an App Group is per-device: `WatchWire.swift` imports no WatchConnectivity,
  which is what lets one `swiftc` process play both devices and makes the whole two-device loop
  provable from `flutter test`. Two targets generated by `tool/ios_watch_target.rb`; `sid` is the
  one new mirror field, and it closes both halves of the shared-phone leak a second device opens

**Gates (Sep 6, watch pass):** `flutter analyze` clean · `flutter test` **1676**
(1605 + 30 iOS-watch pins + 9 two-device Swift round-trips + 4 mirror cases, and
the widget's `t` pin re-aimed at `append(at:)`) · the **paired-simulator loop run
end to end** against production `alastpuff` on an iPhone 17 Pro (iOS 26.3) paired
...
| **S10 Defend** | Feb 1 – Mar 15 | Hold through the seasonal trough. Churn work, annual-mix push, ~~FR/DE/PT ASO — **the app is already fully translated**, so this is listing work only~~ *(listing + site localization **pulled forward to now**, and **Spanish added** — founder decision Sep 19 2026, docs/10 §43: 2 Play installs across 177 countries says availability was never the constraint, findability was)* | Churn ≤ 20% |
...
| **Apple Watch app + complication** — *built Sep 6 2026* | ≤1-tap action | The same stage the widget occupies, and it beats it on the one axis that matters mid-craving: a wrist needs no pocket. Admitted on a founder decision rather than on this table's own test, and it earns its place the cheap way — one shared contract, one new transport, no second implementation of any number. Doc 5 §3 budgeted "a separate native mini-app"; it came in at ~900 lines of Swift because nothing in it does arithmetic. | done |
| **Site + store listings in es / fr / de / pt** — *in progress, Sep 19 2026* | Top of funnel | The app ships five languages and is live in 177 countries; the site and both listings are English-only, so nothing outside English can find it (docs/10 §43: 270 real site visits in 21 days, ~20 from search; 2 Play installs). Homepage, `/download` and both listings first, blog after — top three posts per language. **es and pt are written for the wider audience** (neutral international Spanish, Brazilian Portuguese), which is *not* the app's dialect. Phases and decisions: docs/10 §43. | now |
| **App strings for es-419 / pt-BR** | — (consistency) | Follows from the row above. `app_es.arb` is Peninsular ("móvil", "caladas") and `app_pt.arb` is European ("telemóvel", "a tua", "passas"); a Brazilian who arrives from a pt-BR page and a pt-BR listing opens a pt-PT app. Readable, and noticeably foreign. Flutter's `gen-l10n` takes `app_pt_BR.arb` / `app_es_419.arb` beside the base files, so it is additive — but it is 646 keys × 2 and every name-bearing key is pinned by `coach_name_test.dart`. Not started; do it once the localized pages show which market actually answers. | after S8 |
...
| 9 | Apple Watch — PRD §8 V1.1 vs Doc 5 §3 V2 | **Superseded by a founder decision, Sep 6 2026: built.** Doc 5 was right that it is a separate native mini-app and wrong about the cost — the watch reuses `CirrusShared.swift`/`CirrusOutbox.swift` and the mirror wholesale, so it is a renderer and a transport, not a second app. `ios/CirrusWatch/` + `ios/CirrusWatchComplication/`, docs/10 §29. Neither doc's phase applies any more |
