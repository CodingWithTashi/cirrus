...
- [~] `S3-11` SOS: the 60-min pin is **done**, the panic flow routes into it (composer pre-tagged `sos`), and since Sep 6 2026 a reply notifies the post's author on EVERY tag, not just SOS — collapsed per thread, with `@alias` mentions and a deep link to the thread (docs/10 §29). Still open: notifying the last-5 responders (a fan-out that wants its own decision now the pipeline exists) and the "23 people had your back" count (Doc 3 §9). The buddy half of this line is dropped with the buddy system
