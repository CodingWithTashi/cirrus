...
2. **Every callable sets `enforceAppCheck: true`.** On a debug build the token is a debug secret printed to logcat that **rotates on every install** — and `flutter test integration_test` uninstalls the app when it finishes, so a token registered after a run is already stale. `integration_test/README.md` has the only sequence that works.
...
- `functions/`: `npm run verify` is the deploy gate (typecheck + lint + vitest). `npm run test:rules` and `npm run test:integration` need the Firestore emulator, which needs Java — they run in CI, not on the dev machine.
