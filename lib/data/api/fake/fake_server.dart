...
class FakeServer {
  ...
  /// Inbox rows, newest first, each carrying the account it belongs to.
  final List<Map<String, dynamic>> _notifications = [];

  /// Fires whenever [_notifications] changes.
  ///
  /// A change signal rather than a poll, because the alternative leaked a
  /// periodic timer into every widget test that pumps the app — the same trap
  /// the connectivity probe carries a nullable interval provider for. There is
  /// nothing to poll anyway: the store being watched is in memory in this
  /// isolate, so it can simply say when it moved.
  final _notificationsChanged = StreamController<void>.broadcast();

  Stream<void> get notificationsChanged => _notificationsChanged.stream;
  ...
  /// The session account's notification inbox, newest first.
  ///
  /// Generated by the same events the real backend generates them from — a
  /// reply landing on a post this account wrote — rather than seeded. A seeded
  /// one would render to the reader as something that happened to them, which
  /// is the rule this app breaks hardest when it breaks it at all.
  List<Map<String, dynamic>> notificationsForSession() => [
    for (final n in _notifications)
      if (n['owner'] == _sessionAccountId) _copy(n)!,
  ];

  /// Files a notification for the account that owns [postId], if anyone does.
  ///
  /// Keyed by thread like the server's, so a busy post is ONE row that
  /// updates rather than one per reply.
  void notifyPostAuthor(String postId, Map<String, dynamic> notification) {
    final owner = _postAuthors[postId];
    if (owner == null) return;
    final id = notification['id'] as String;
    _notifications.removeWhere(
      (n) => n['id'] == id && n['owner'] == owner,
    );
    _notifications.insert(0, {...notification, 'owner': owner});
    _notificationsChanged.add(null);
  }

  void markNotificationsRead(List<String> ids, int readAtMs) {
    for (final n in _notifications) {
      if (n['owner'] == _sessionAccountId && ids.contains(n['id'])) {
        n['readAtMs'] = readAtMs;
      }
    }
    _notificationsChanged.add(null);
  ...
  /// One post by id, under exactly the visibility [postsForSession] applies.
  ///
  /// Reuses that method rather than re-deriving the rule: the fake's whole
  /// job is to answer like the real backend, and two copies of "who may see
  /// this" would drift the moment one of them changed.
  Map<String, dynamic>? postById(String postId) {
    for (final p in postsForSession()) {
