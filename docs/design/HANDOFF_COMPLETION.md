...
| 52 | Widgets + states | empty-stats state ✅ · offline pill + error card ✅ (`OfflineBanner`, `LpErrorState`, mounted app-wide) · **Android home-screen widget ✅ Sep 4 2026** — one resizable provider, day number above the count, `+`/`−` that log while the app is dead · **iOS widget ✅ Sep 5 2026** — `ios/CirrusWidget/` built, embedded and driven on an iPhone 16 Pro simulator (small + medium; the lock-screen accessory families render from the same views but await a hand pass on a phone). **Android has no phone lock-screen widget**, so that half of the frame is iOS-only | ✅ / ✅ / ✅ |
