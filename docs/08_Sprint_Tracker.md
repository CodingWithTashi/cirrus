...
| ~~**B21**~~ | [HALF RESOLVED Sep 4 2026] **The widget exists on Android**, so the listing's two claims are now true for every Android install — see `B14`. **Still open until iOS ships:** the description does not say "Android", so an iPhone reader is still promised something that is not there. Two ways to close it: land the iOS extension (the Swift sources and the App Group plumbing are written and committed; what remains is Xcode target creation on the Mac), or qualify the sentence in the listing until it does. **Founder call.** | live listing `com.quitvape.last_puff`; `ios/CirrusWidget/` | S5 · founder |
| **B22** | **The iOS widget is authored but unbuilt.** `ios/CirrusWidget/` holds the WidgetKit bundle, the timeline provider, the iOS-17 `AppIntent` for one-tap logging and the App Group entitlements; the Dart side is already platform-agnostic, so the mirror and the outbox work the moment the target exists. **What no file edit can do, and only the Mac can:** register App Group `group.com.quitvape.lastPuff` and App ID `com.quitvape.lastPuff.CirrusWidget` in the developer portal (this must happen BEFORE the entitlements change reaches a machine that builds, or the host app itself fails to sign), regenerate both provisioning profiles, `pod install`, and archive from `Runner.xcworkspace`. **Read `ios/CirrusWidget/README.md` first** — it carries the ordered 10-step checklist, a file-by-file inventory, the three bugs a code review already fixed in the Swift, and the two build traps (`@main` collides with Xcode's own template; the version keys resolve only through `Generated.xcconfig`). The Swift has never been compiled — expect a first pass of ordinary build errors; the logic has been reviewed, the syntax has not. | `ios/CirrusWidget/`, `ios/Runner/Runner.entitlements` | iOS fast-follow · founder |
