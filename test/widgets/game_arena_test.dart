...
void main() {
  ...
  /// Straight to the arena. Never `pumpAndSettle` past this point: the
  /// field animates until the round ends.
  Future<void> openGame(
    WidgetTester tester,
    ProviderContainer c, {
    String? path,
  }) async {
    c.read(routerProvider).go(path ?? Routes.game);
    await tester.pump();
  ...
  group('the arena', () {
    testWidgets('opens on the last game played, Tiles the first time', (
      tester,
    ) async {
      final (c, _) = await boot(tester);
      await openGame(tester, c);
      expect(find.byType(TileField), findsOneWidget);
      expect(c.read(quitStoreProvider)!.lastGame, GameId.tiles);
