...
`npm run check` 0 errors / 29 files · `npm run build` 17 pages · `npm run verify` red on the FAQ collision first, then **17 pages, 368 internal links, 51 FAQ questions — all consistent** · `/get` exercised on the local Pages runtime · CLS measured in Chrome, production against the new build. **Not done:** the phone-width pass — the Pixel 8 was locked, so nothing was seen on the device; no preview deploy yet; and none of the Cloudflare zone changes (POST block, Bot Fight Mode, DMARC, TLS 1.2, Crawler Hints) — each waits for a yes.
